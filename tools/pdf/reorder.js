...
// tools/pdf/reorder.js — reorder pages via order spec like "3,1,2" or "2-4,1". (P2 shell UI.)
...
export async function mount(el, ctx = {}) {
  ...
  page.optionsEl.innerHTML = `
    ...
    <label>New order (e.g. 3,1,2 or 2-4,1)
      <input type="text" data-f="order" placeholder="3,1,2" />
    </label>`;
  ...
  page.runBtn("Reorder & download", async (got) => {
    try {
      files = [...(got || [])];
      const f = files[0];
