...
// tools/pdf/grayscale.js — raster grayscale: render each page via pdf.js,
// ctx.filter grayscale(1) → JPEG → pdf-lib rebuild (pixels burned).
export async function mount(el, ctx = {}) {
  el.innerHTML = `
    <div style="display:grid;gap:10px">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap"><button data-f="go">Grayscale & download</button></div>
      <p style="font-size:12px;color:#64748b;margin:0">Grayscale burns pages to pixels (print-friendly). Text is no longer selectable and color cannot be restored.</p>
  ...
  const TOOL_ID = (ctx && ctx.tool && ctx.tool.id) || "pdf-grayscale";
  const trackedUrls = [];
  ...
  function toGrayscale(srcCanvas, dstCanvas) {
    const g2d = dstCanvas.getContext("2d");
    g2d.fillStyle = "#fff";
    g2d.fillRect(0, 0, dstCanvas.width, dstCanvas.height);
    let filtered = false;
    try {
      if ("filter" in g2d) {
        g2d.filter = "grayscale(1)";
        g2d.drawImage(srcCanvas, 0, 0);
        // If the engine accepted the filter value, pixels are burned gray.
        if (g2d.filter === "grayscale(1)" || g2d.filter === "grayscale(100%)") filtered = true;
      }
    } catch { filtered = false; }
    if (!filtered) {
      // Manual luminance fallback for engines without ctx.filter.
      try { g2d.filter = "none"; } catch {}
      g2d.drawImage(srcCanvas, 0, 0);
      const img = g2d.getImageData(0, 0, dstCanvas.width, dstCanvas.height);
      const d = img.data;
  ...
  const onGo = async () => {
    ...
    try {
      ...
      if (pdf.numPages > rasterMax) throw new Error(`Page-count guard: PDF has ${pdf.numPages} pages (cap ${rasterMax} for grayscale on ${deviceLabel}). Try Split into <=${rasterMax}-page parts, or use Desktop (cap ${RASTER_MAX_DESKTOP}).`);
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        try {
          status(`Grayscaling page ${i}/${pdf.numPages} (JPEG q${JPEG_Q})…`);
          ...
          try {
            ...
            if (pixels > RASTER_MAX_PIXELS || vw > maxDim || vh > maxDim) {
              ...
              status(`Grayscaling page ${i}/${pdf.numPages} downscaled to ${vw}×${vh} (JPEG q${JPEG_Q})…`);
            }
            ...
            liveCanvases.add(grayCanvas);
            grayCanvas.width = vw;
            grayCanvas.height = vh;
            toGrayscale(tmpCanvas, grayCanvas);
            const dataUrl = grayCanvas.toDataURL("image/jpeg", JPEG_Q);
          ...
          tmpCanvas = null;
          try { if (grayCanvas) { grayCanvas.width = 0; grayCanvas.height = 0; liveCanvases.delete(grayCanvas); } } catch {}
          grayCanvas = null;
        } catch (e) {
          if (e && e.message && e.message.startsWith("Page-count guard:") === false) {
            ...
            if (/memory|allocation|array buffer|OOM|Maximum call|out of/i.test(msg)) {
              throw new Error(`Out of memory grayscaling page ${i}/${pdf.numPages} (${msg}). Try Split into <=${rasterMax}-page parts.`);
            }
          }
          throw e;
        }
      ...
      saveBytes(bytes, file.name.replace(/.pdf$/i, "") + "-grayscale.pdf", "application/pdf");
      status(`Done — grayscaled ${pdf.numPages} pages (JPEG q${JPEG_Q}). ${origLen} → ${bytes.length} bytes. Pixels burned: text is image-only.`);
      ...
      try {
        ...
        if (grayCanvas) { grayCanvas.width = 0; grayCanvas.height = 0; liveCanvases.delete(grayCanvas); }
      } catch {}
