...
// tools/pdf/target-size.js — shrink PDF to ~1/2/5MB via raster JPEG quality loop.
export async function mount(el, ctx = {}) {
  el.innerHTML = `
    <div style="display:grid;gap:10px">
      ...
      <label style="font-size:13px;font-weight:600">Target size
      ...
      <div style="font-size:12px;color:#64748b">Best-effort: rasterizes pages to JPEG, stepping quality down until under target (±30% tolerance, else warns with closest).</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap"><button data-f="go">Shrink & download</button></div>
  ...
  const onGo = async () => {
    ...
    try {
      ...
      try { prog.value = prog.max; pct.textContent = "100%"; } catch {}
      if (met) {
