...
/**
 * tests/e2e/vectorize-smoke.spec.js — E2E smoke test for Vectorization.
...
test.describe('image-vectorize', () => {
  ...
  test('has mode, color, and hierarchical selectors', async ({ page }) => {
    await expect(page.locator('#view select[data-f="mode"]')).toBeVisible();
    await expect(page.locator('#view select[data-f="colorMode"]')).toBeVisible();
    await expect(page.locator('#view select[data-f="hierarchical"]')).toBeVisible();
  });
